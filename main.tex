...
\begin{enumerate}
    ...
    \item No corpus Bosque-UD, a coluna “xpos” (classe gramatical específica) é preenchida com a saída do sistema PALAVRAS \citep{bick2000parsing} para a mesma frase;
...
\chapter{Lemas}\label{sec:lemas}
	...
	O lema é a terceira coluna (ou atributo) no formato CoNLL-U. De modo geral, os lemas não são capitalizados, isto é, devem ter letras minúsculas, exceto pelos nomes próprios (\emph{PROPN}).
...
São 17 as classes gramaticais em UD. Elas são decididas com base no contexto, seguindo o \fullref{sec:classesdinamicas}.
...
\section{Verbos auxiliares}\label{sec:verbosauxiliares}
	...
	Verbos auxiliares são classificados como \emph{AUX}. Via de regra, verbos auxiliares (\emph{AUX}) não podem ter dependentes, e dependem de um verbo principal (\emph{VERB}). O deprel pode ser \emph{cop}, \emph{aux} ou \emph{aux:pass}, de acordo com a função sintática.
	...
	\subsection{Verbos de ligação}\label{sec:verbosdeligacao}
		...
		Os verbos de ligação \textit{AUX} terão relação sintática \emph{cop}, e nunca poderão ser núcleo de uma oração nem conter dependentes.
	...
	\subsection{Locuções verbais aspectuais/modais não existem no UD}\label{sec:auxaspecto}
		...
		Em UD, não existem locuções verbais aspectuais ou modais, de modo que as sentenças são anotadas como na \fullref{dep:locverbalUD} e \fullref{dep:locverbalUD2}, com dois verbos plenos (\emph{VERB}), sendo o segundo de deprel \emph{xcomp} dependente do primeiro.
	...
	\subsection{Proposta de anotação para as locuções verbais aspectuais/modais}\label{sec:locverbalJDP}
		...
		Em UD, não existem locuções verbais aspectuais ou modais, mas estudamos uma alternativa para marcar a auxiliaridade dos verbos segundo alguns estudos gramaticais.
		...
		Alguns gramáticos divergem sobre como a classificação se dá, portanto propomos herdar, no Bosque-UD, o que já era a anotação originária do PALAVRAS \citep{bick2000parsing}, e não modificá-la em relação a quais devem ser os verbos auxiliares.
		
		Para o PALAVRAS (no Bosque-UD, vigente até a versão 2.4), o que por vezes é chamado de locuções verbais modais não são locuções, isto é, os dois verbos se configuram como verbos plenos, tal como é a anotação seguindo as diretivas do UD (\fullref{dep:locverbalUD}).

		Nos casos de locução verbal aspectual, porém, para o PALAVRAS, há uma locução, isto é, o primeiro verbo é auxiliar (\textit{AUX/aux}), e o segundo, verbo pleno (\textit{VERB}). Às vezes há uma partícula interveniente no meio da locução, como na \fullref{dep:auxphrasalverb}, em que há um \say{a} entre \say{começar} e \say{ouvir}. Encaramos que estamos diante de um fenômeno de \emph{phrasal verb}, uma MWE do tipo \emph{AUX}, como sugerido em \citet{elvis2019locverbal}. Na frase, \emph{vai ouvir} é uma locução verbal de tempo composto (\fullref{sec:auxtempocomposto}), e \say{começar a ouvir} é uma locução verbal aspectual, sendo \say{começar a} uma expressão multi-palavra (MWE) do tipo auxiliar.
...
\section{Numerais}\label{sec:numerais}
	...
	Numerais cardinais (360) e fracionários (\fullref{dep:numfrac}) devem ser anotados como de upos \emph{NUM}, sejam por extenso ou em algarismos. Numerais multiplicativos (dobro, triplo), por outro lado, devem ser anotados como de upos \emph{NOUN}.
	...
	\subsection{Números compostos}\label{sec:numeroscompostos}
		...
		Números compostos podem conter upos \emph{NUM}, \emph{CCONJ} e \emph{ADP}. Quando o número composto modifica um substantivo, o primeiro número deve se subordinar ao substantivo como de deprel \emph{nummod}; quando há um sintagma preposicionado, o primeiro número do número composto será o head, e o substantivo, \emph{nmod} do número. Além disso, deve-se notar que o primeiro número deve receber no misc \emph{MWE} e \emph{MWEPOS}.
...
As preposições são palavras que operam como conectores de termos nominais.
	...
	\subsection{Princípio de classes dinâmicas}\label{sec:classesdinamicas}
	
		Palavras tidas usualmente como preposições na GT podem operar como conjunções subordinativas quando em contexto de introdução de oração (\fullref{dep:classedinamica}). Ou seja, a classe gramatical é dada pelo contexto, não existindo preposição estática. Trata-se de uma decisão provisória, mas que tem sido aplicada consistentemente no Bosque-UD. Há apenas uma exceção: o caso das conjunções coordenativas (\emph{CCONJ}), que são sempre de deprel \emph{cc}, como, por exemplo, a conjunção aditiva \say{e} (\fullref{sec:cconj}).
		...
		\begin{figure*}[htbp]
					...
					\begin{dependency}
						...
						\depedge{6}{5}{expl}
						\depedge{3}{6}{acl}
					\end{dependency}\label{dep:classedinamica}
...
\section{Conjunções}\label{sec:conjuncoes}
	...
	Há dois tipos de conjunções no Bosque-UD: conjunção coordenativa (\emph{CCONJ}) e conjunção subordinativa (\emph{SCONJ}). As funções que apresentam são \emph{cc} (coordenador) e \emph{mark} (subordinador).

	As locuções coordenativas e subordinativas frequentemente aparecem como expressões multi-palavras (MWE). Nesses casos, o primeiro token deve receber o mesmo upos (\emph{SCONJ/CCONJ}) e deprel (\emph{cc/mark}), e os demais tokens devem receber deprel \emph{fixed} para o primeiro. Além disso, deve-se adicionar à coluna misc os valores \emph{MWE} e \emph{MWEPOS}.
	...
	\subsection{Conjunções subordinativas}
		...
		É importante lembrar que palavras tidas usualmente na GT como preposição, ao introduzir orações, são classificadas como conjunções subordinativas e recebem o deprel \emph{mark} (ver \fullref{sec:classesdinamicas}).
	...
	\subsection{Advérbios interrogativos}\label{sec:adverbiosinterrogativos}
		...
		Os advérbios interrogativos podem operar como conjunções subordinativas quando aparecem na introdução de orações subordinadas adverbiais, conforme \fullref{sec:classesdinamicas}.
...
\section{Núcleo da oração}

	\begin{enumerate}

		\item O predicado é verbal quando o núcleo da oração tem upos \emph{VERB}. Sua deprel deverá ser, portanto, a função sintática da oração: seja ela principal (\emph{root}), subordinada ou coordenada (\fullref{sec:coordenacao}). Caso seja subordinada, qualquer uma das funções sintáticas poderá se aplicar.
	
		\item O predicado é nominal quando o verbo é de ligação (\fullref{sec:verbosdeligacao}). Nesses casos, o verbo deve ter upos \emph{AUX} e deprel \emph{aux}, sendo dependente do sintagma nominal que o verbo introduz. Este sintagma nominal, por sua vez, será o núcleo da oração.

		\item O predicado é verbo-nominal quando o núcleo da oração é um verbo pleno, de upos \emph{VERB}, mas há espaço para um predicativo do objeto. É o caso, por exemplo, de \fullref{sec:pintouamodelonua}.

	\end{enumerate}

\section{Coordenação}\label{sec:coordenacao}

	A coordenação acontece quando duas sentenças não são subordinadas uma à outra, mas são independentes. É possível falar, também, em coordenação de sintagmas nominais, e não apenas verbais.

	Em todo tipo de coordenação, é necessário uma conjunção coordenativa, de upos \emph{CCONJ} e deprel \emph{cc}, dependente de um núcleo de coordenação, que deve receber deprel \emph{conj}. O núcleo da coordenação, por sua vez, é dependente do núcleo da oração à qual é coordenado.

	Ver \fullref{sec:cconj}

	Repare que, em alguns casos, coordenação de apostos pode não ser anotada como \emph{conj}, conforme \fullref{sec:apostoscoordenados}.
...
\subsection{Caso o sujeito seja nominal}
	...
	\subsection{Caso o sujeito seja oracional} 
...
\section{Predicativos}\label{sec:predicativos}
	...
	Os predicativos do sujeito, quando são o predicado nominal da oração, devem ter deprel \emph{root} como na \fullref{dep:predicativodosujeito1}.

	Quando são adjunto do sujeito, os predicativos são adjuntos adnominais, conforme \fullref{sec:adjuntoadnominal}, e devem receber deprel \emph{acl}, \emph{nmod}, \emph{amod} ou \emph{nummod}.
	...
	\subsection{Pintou a modelo \emph{nua}: predicativo do objeto subordinado ao objeto}\label{sec:pintouamodelonua}
		...
		Por outro lado, caso o predicativo seja dependente do verbo, será \emph{xcomp}, conforme \fullref{sec:declarouoreuculpado}.
	...
	\subsection{Declarou o réu \emph{culpado}: predicativo do objeto subordinado ao verbo}\label{sec:declarouoreuculpado}
		...
		Por outro lado, caso o predicativo seja dependente do objeto, será \emph{acl}, conforme \fullref{sec:pintouamodelonua}.
...
\section{Argumentos do verbo}\label{sec:argumentosdoverbo}
	...
	\subsection{Caso o argumento do verbo seja nominal}\label{sec:objetonominal}
	...
	\subsection{Caso o argumento do verbo seja oracional}\label{sec:objetooracional}
		...
		No entanto, em alguns casos, o objeto oracional pode ser \emph{xcomp}, quando o sujeito da oração subordinada é, obrigatoriamente, o mesmo sujeito da oração principal, ou o objeto da oração principal. De modo geral, são os casos de oração subordinada substantiva objetiva reduzida, como em \fullref{dep:xcomp1}, \fullref{dep:xcomp2}, \fullref{dep:xcomp3}, e \fullref{dep:xcomp4}. 
...
\section{Adjunto do verbo}\label{sec:adjadv}

	O adjunto do verbo é o adjunto adverbial. Ele terá anotação diferente caso seja nominal, isto é, o adjunto adverbial clássico, ou oracional, isto é, uma oração subordinada adverbial.
	...
	\fullref{sec:adjadvverb}
		...
		\subsection{Caso o adjunto adverbial seja nome}\label{sec:adjadvnome}
		...
		\subsection{Caso o adjunto adverbial seja verbo}\label{sec:adjadvverb}
...
\section{Discurso direto}\label{sec:discursodireto}
	...
	\begin{figure}
	    \centering
	    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{imagesDrive/image10.png}
	...
	\begin{figure}
	    \centering
	    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{imagesDrive/image70.png}
