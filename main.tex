...
As preposições são palavras que operam como conectores, geralemente, de termos nominais.

Entretanto, no Bosque-UD, palavras tidas usualmente como preposições podem operar como conjunções subordinativas quando em contexto de introdução de oração (\fullref{dep:classedinamica}).
...
\begin{figure*}[htbp]
			...
			\begin{dependency}
				\begin{deptext}
					CCONJ \& PROPN \& CCONJ \& DET \& NOUN \& ADV \& VERB \& DET \& NOUN \& SCONJ \& PRON \& 						VERB \\
					Nem \& Lula \& nem \& o \& partido \& ainda \& encontram \& um \& discurso \& para \& se \& 							diferenciar \\
				\end{deptext}
				\deproot{7}{root}
				\depedge{7}{9}{obj}
				\depedge{12}{10}{mark}
			...
			\caption{Nem Lula nem o partido ainda encontraram um discurso \emph{para} se diferenciar}
		\end{figure*}

	\subsection{Princípio de classes dinâmicas}

\section{Conjunções}

Há dois tipos de conjunções no Bosque-UD: conjunção coordenativa (CCONJ) e conjunção subordinativa (SCONJ). As funções que apresentam são \emph{cc} (coordenador) e \emph{mark} (subordinador). As locuções coordenativas e subordinativas frequentemente aparecem como expressões multi-palavras (MWE) e têm a mesma função que as conjunções coordenativas (\emph{cc/mark}), além da mesma classe gramatical (\emph{CCONJ/SCONJ}).
...
Além disso, também são consideradas conjunções casos em que não há uma oração. Toda vez que houver a coordenação entre dois ou mais termos, sejam eles oracionais ou nominais, o conectivo entre eles será classificado como conjunção, e o núcleo da coordenação deverá ter deprel \emph{conj}.
...
\begin{figure*}[htbp]
			...
			\end{dependency}
			\caption{\emph{Se} eu dirigisse uma federação, apresentaria balanços mensais e liberaria minhas contas bancárias}
		\end{figure*}
...
\begin{figure*}[htbp]
			...
			\begin{dependency}
				...
				\deproot{7}{root}				
			\end{dependency}
			\caption{Mas, \emph{mesmo que} quisessem não conseguiriam fazer nada menor}
		\end{figure*}
