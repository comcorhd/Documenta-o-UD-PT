...
\section{Verbos auxiliares e verbo de ligação}\label{sec:verbosauxiliares}
	...
	\subsection{Locuções verbais de tempo composto}\label{sec:auxtempocomposto}
		...
		Segundo as gramáticas, são locuções verbais de tempo composto aquelas que têm como verbo auxiliar \say{estar}, \say{ter}, \say{haver} e, para nós, também \say{ir}.
	...
	\subsection{O importante \emph{é} manter a calma: verbo \textit{ser} como verbo pleno}\label{sec:serpleno}
	
		Atenção para um caso em que o \say{ser} não é verbo auxiliar e deve ser verbo pleno (upos \textit{VERB}).
		
		Como na \fullref{dep:serVERB}, o \say{ser} deve ser núcleo da oração caso o predicado seja uma outra oração (\textit{ccomp, xcomp}).
...
\section{Artigos, pronomes substantivos e pronomes adjetivos}\label{sec:pronedet}
	...
	\subsection{Pronomes indefinidos}\label{sec:pronind}
		...
		Pronomes indefinidos devem receber o valor \emph{PronType=Ind} na coluna feats, e podem ser pronomes substantivos (upos \emph{PRON}), como na \fullref{dep:pronindsubst}, ou adjetivos (upos \emph{DET}), como na \fullref{dep:pronindadj}.
...
\section{Adjuntos e argumentos do substantivo}\label{sec:adjuntoseargumentosdosubstantivo}
	...
	\subsection{Aposto}\label{sec:aposto}
		...
		Em UD, a relação \emph{appos} é utilizada para \say{modificador apositivo} (\emph{appositional modifier}), e sua utilização é mais restrita do que a do aposto segundo GTs da língua portuguesa - que também oferecem um tratamento bastante heterogêneo desta classe de fenômenos. Além disso, em UD, a etiqueta \emph{appos} tem uma restrição formal - só pode ser usada se o elemento \emph{appos} está imediatamente após o seu \say{núcleo}.

		Em UD, são analisados como aposto apenas os casos que a GT convencionou chamar de aposto explicativo (\fullref{dep:apostonominal1}). Os chamados apostos especificativos são analisados como \emph{nmod} (\fullref{sec:nomenmod}). O corpus Bosque-UD (até a versão 2.7) ainda analisa o aposto especificativo como \emph{appos}, mas esta classificação não está de acordo com as decisões de UD sobre o aposto (\href{https://universaldependencies.org/u/dep/appos.html}{appos} e \href{https://github.com/UniversalDependencies/docs/issues/591}{Issue \#591}).

		Também estão em relação de aposto endereços de internet e e-mails, como em \fullref{dep:email}, quando há a presença dos dois pontos. Caso não haja pontuação, a classificação será \emph{nmod} (Ver \fullref{sec:nomeseguidodenome}).
		
		Apostos oracionais, como na \fullref{dep:apostooracional}, não têm uma análise unânime em UD, uma vez que, até o momento, apostos são considerados elementos da família \emph{nmod} e, portanto, precisam ser dependentes de um elemento nominal, e não de um verbo (ver a discussão em \href{https://github.com/UniversalDependencies/docs/issues/762}{Issue \#762}). Até o momento, apesar das restrições e considerando a ausência de uma solução consensual, continuamos analisando apostos oracionais como \emph{appos}, e, para encontrar esses casos e substituí-los no futuro por alguma outra análise, basta procurar por \emph{appos} que sejam dependentes de um verbo. 
...
\section{Relações que só existem em UD}
	...
	\subsection{fixed, compound, flat:name (MWE)}\label{sec:mwe}
		...
		\begin{figure}[H]
			...
			\begin{dependency}
				...
				\deproot{2}{root}
			\end{dependency}
			\caption{o aumento do efeito \emph{estufa}}
			\label{dep:compound}
		\end{figure}
	...
	\subsection{flat:foreign}

		\emph{flat:foreign} é a deprel utilizada quando há termos em língua estrangeira, isto é, diferente da língua majoritária do corpus
...
\hyperlink{toc}{Ir para tabela de conteúdos\\}
	...
	\section{O que fazer com...}\label{sec:quefazer}
		...
		\subsection{Endereços de internet, e-mails etc.}
			...
			Endereços de internet, e-mails etc. são anotados como \emph{NOUN}. Com relação à função sintática, podem ser \emph{nmod} ou {appos}, dependendo da pontuação que os acompanham (Ver \fullref{sec:aposto}).
		...
		\subsection{Nome seguido de nome: \emph{appos}, \emph{compound}, \emph{flat:name} ou \emph{nmod/amod}?}\label{sec:nomeseguidodenome}
			...
			\subsubsection{\emph{appos}}\label{sec:nomeappos}
				...
				Utilizado nos casos típicos em que o segundo nome especifica um tipo do primeiro nome; em casos de abreviação. Ver também: \fullref{sec:aposto}

				empresa Doris Engenharia, \emph{subsidiária} da Doris Engineering,

				sócios-gerentes da empresa, \emph{profissionais} com ampla experiência em projetos

				altas concentrações de compostos aromáticos (\emph{BTX} 209 µg/g e HPAs 1093 µg/g)

				ramnolipídeo, um \emph{surfactante} biodegradável com aplicações em diversas áreas

				Fundação Universidade Federal de o Rio Grande (\emph{FURG})

				hexametilendiamina (\emph{HMDA})

				Laboratório de Biotecnologia Microbiana (\emph{LaBiM})

				Petróleo Brasileiro S/A (\emph{Petrobras})

				HPAs (\emph{hidrocarbonetos} poliaromáticos)
			...
			\subsubsection{\emph{compound}}\label{sec:nomecompound}
				...
				poli(\emph{bisfenol} \emph{A-co-epicloridrina})

				1,2 milhão de barris \emph{/ dia} (\say{barris} é \emph{nmod} de \say{milhão})

				tert-butanol \emph{60 \%}

				Ver também: \fullref{sec:mwe}

			\subsubsection{\emph{flat:name}}\label{sec:nomeflatname}

				Utilizado quando um nome próprio modifica outro nome próprio; sem sintaxe interna; em casos de coordenadas geográficas
			...
			\subsubsection{\emph{nmod} e \emph{amod}}\label{sec:nomenmod}

				São utilizados na maioria dos casos de nomes que modificam nomes; no caso de substantivo (\emph{nmod}), há a preposição \say{de} ou ela está omitida

				emulsões \emph{base} \emph{éster} (\say{éster} é \emph{nmod} de \say{base})
	
				navio \emph{tipo} \emph{A} (\say{A} é \emph{nmod} de \say{tipo})
				...
				Decreto \emph{n.º} 5.297 (os algarismos são \emph{nummod} de \say{nº})
	
				Lei \emph{nº} 11.097 (os algarismos são \emph{nummod} de \say{nº})
